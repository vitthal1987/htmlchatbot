...
# Function to calculate RSI (With Fix for Empty Data)
def calculate_rsi(stock_code, period=14):
    ...
    # Avoid index error by checking if we have enough data
    if len(df) < period:
...
def calculate_pivot_points(stock_code):
    ...
    if df.empty:
        return {"Pivot": "N/A", "Resistance 1": "N/A", "Support 1": "N/A", "Resistance 2": "N/A", "Support 2": "N/A"}
...
def get_stock_analysis():
    ...
    # If RSI function returned an error message, show it
    if isinstance(rsi, str):  
        return render_template("result.html", stock=stock_name.upper(), rsi=rsi, pivot={}, sentiment="N/A", trend_score="N/A", news=[])
    ...
    return render_template("result.html", stock=stock_name.upper(), rsi=rsi,
